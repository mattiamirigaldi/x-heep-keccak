...
void PQCLEAN_KYBER512_CLEAN_gen_matrix(polyvec *a, const uint8_t seed[KYBER_SYMBYTES], int transposed) {
    ...
    xof_state state;
    //printf("indcpa : starting gen matrix. KYBER_K : %d.\n",KYBER_K);
    for (i = 0; i < KYBER_K; i++) {
      //printf("Iteration i: %d,  Transposed : %d\n",i, transposed);
        for (j = 0; j < KYBER_K; j++) {
...
void PQCLEAN_KYBER512_CLEAN_indcpa_keypair(uint8_t pk[KYBER_INDCPA_PUBLICKEYBYTES],
    ...
    //printf("indcpa_keypair : pre random\n");
    randombytes(buf, KYBER_SYMBYTES);
    //printf("indcpa_keypair : pre hash\n");
    hash_g(buf, buf, KYBER_SYMBYTES);
    //printf("indcpa_keypair : pre gen_a\n");
    gen_a(a, publicseed);

    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_KYBER512_CLEAN_poly_getnoise_eta1(&skpv.vec[i], noiseseed, nonce++);
    }
    //printf("indcpa_keypair : post poly_getnoise sk\n");
    for (i = 0; i < KYBER_K; i++) {
        PQCLEAN_KYBER512_CLEAN_poly_getnoise_eta1(&e.vec[i], noiseseed, nonce++);
    }
    //printf("indcpa_keypair : post poly_getnoise evec\n");
    ...
    //printf("indcpa post matrix vector multiplication\n");

    PQCLEAN_KYBER512_CLEAN_polyvec_add(&pkpv, &pkpv, &e);
    PQCLEAN_KYBER512_CLEAN_polyvec_reduce(&pkpv);
    
    //printf("indcpa pre pack sk and pk\n");
    pack_sk(sk, &skpv);
    pack_pk(pk, &pkpv, publicseed);
    //printf("indcpa returning\n");
}
