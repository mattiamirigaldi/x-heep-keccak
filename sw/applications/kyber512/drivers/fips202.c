...
static void keccak_absorb(uint64_t *s, uint32_t r, const uint8_t *m,
    ...
    size_t i;
    uint8_t t[200];
    //printf("fips202 : starting keccak absorb\n");
    /* Zero state */
    for (i = 0; i < 25; ++i) {
        s[i] = 0;
    }
	...
	//printf("fips202 : Keccak call\n");
        KeccakF1600_StatePermute(s, s);
	//printf("fips202 : Keccak call return\n");
	mlen -= r;
        m += r;
    }
    ...
    //printf("fips202 : Finished keccak_absorb \n");
...
void shake128_absorb(shake128ctx *state, const uint8_t *input, size_t inlen) {
    //printf("fips202 : starting shake128_absorb\n");
    state->ctx = malloc(PQC_SHAKECTX_BYTES);
    if (state->ctx == NULL) {
        exit(111);
    }
    //printf("fips202 : calling keccak_absorb\n");
    keccak_absorb(state->ctx, SHAKE128_RATE, input, inlen, 0x1F);
    //printf("fips202 : finished shake128_absorb\n"); 
}
...
void sha3_512(uint8_t *output, const uint8_t *input, size_t inlen) {
    ...
    //printf("fips202 : sha3_512 \n");
    

    for (size_t i = 0; i < 64; i++) {
        output[i] = t[i];
    }
    //printf("fips202 : sha3_512 finished\n");
}
